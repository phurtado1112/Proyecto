...
public class EstructuraEvaluacion {
    ...
    public int validarRegistros(int idactividad, int actividaddet) {
        int cantidad = 0;
        cnx.Conecta();
        try {
            String SQL = "select count(*) from estructuraevaluacion where idactividad=" + idactividad + " and idactividaddet=" + actividaddet + "";
            ps = cnx.conn.prepareStatement(SQL);
            rs = ps.executeQuery();
