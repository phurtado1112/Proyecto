...
public class SesionInicio extends JFrame {
    ...
    private void validarDocente() {
        ...
        if(this.txtUsuario.getText().equals("")){
            ...
            try{
                ...
                JOptionPane.showMessageDialog(null, "Error Validar docente: " + e.getMessage());
            } finally {
                cnx.Desconecta();
            }       
        }    
    ...
    public final boolean comprobarDocente(){
        boolean prueba = false;
        cnx.Conecta();
        try{
            st= cnx.conn.createStatement();
            rs = st.executeQuery("select count(*) as valor from docente");
            rs.next();
